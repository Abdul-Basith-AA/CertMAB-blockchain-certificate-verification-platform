...
for filename in os.listdir(directory):
    if filename.endswith('.html') and filename != 'home_page.html':
        ...
        # Generic match for the logo box above the main heading in login forms
        # Usually something like <div class="w-20 h-20 bg-indigo-600 rounded-2xl ..."><svg...>...</svg></div>
        pattern = r'(<div class="[^"]*w-[0-9]+ h-[0-9]+[^"]*bg-[^"]*rounded-[^"]*flex items-center justify-center[^"]*">)\s*<svg[^>]*>.*?</svg>\s*(</div>)'
